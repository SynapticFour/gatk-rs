...
cases_tsv="${repo_root}/parity/fixtures/hc-full-parity/i1-depth-hc/cases.tsv"
tmp_dir="${repo_root}/parity/reports/hc-full-parity-i1-depth-hc-tmp"
mkdir -p "${tmp_dir}"
...
while IFS=$'\t' read -r case_id ad_csv expected; do
  [[ -z "${case_id}" || "${case_id}" == \#* ]] && continue
  actual="${tmp_dir}/${case_id}.actual.tsv"
  "${cargo_run[@]}" depth-per-sample-hc "${ad_csv}" >"${actual}" 2>"${tmp_dir}/${case_id}.stderr"
  cmp -s "${repo_root}/${expected}" "${actual}" || {
    echo "[hc-full-parity-i1-depth-hc] mismatch case=${case_id}" >&2
    diff -u "${repo_root}/${expected}" "${actual}" >&2 || true
    exit 1
  }
  echo "[hc-full-parity-i1-depth-hc] ok ${case_id}"
done <"${cases_tsv}"

echo "[hc-full-parity-i1-depth-hc] all cases match goldens."
