#!/usr/bin/env bash
# Phase 3 indexed region query edge matrix: Java CountReads vs Rust CountReadsInRegion.
# Uses parity/fixtures/p3_region_reads.sam (LN=32, four 8M reads at POS 1,5,12,20).
set -euo pipefail

repo_root="$(cd "$(dirname "$0")/../.." && pwd)"
report_dir="${repo_root}/parity/reports"
mkdir -p "${report_dir}"
tmp_dir="${report_dir}/p3-indexed-edge-diff-tmp"
mkdir -p "${tmp_dir}"
...
{
  "label": "${label}",
  "mode": "phase3-indexed-edge-query-runtime-diff",
  "equal": null,
  "skipped": true,
...
run_fixture_cases() {
  ...
  # Full contig.
  run_case "p3-edge-query-${fixture_name%.sam}-full-contig" "${source_bam}" "${fixture_name}" "chr1:1-32"
  # Tail interval expected sparse/empty across fixtures.
  run_case "p3-edge-query-${fixture_name%.sam}-empty-tail" "${source_bam}" "${fixture_name}" "chr1:28-32"
  # Single 1-based position at left end.
  run_case "p3-edge-query-${fixture_name%.sam}-single-start" "${source_bam}" "${fixture_name}" "chr1:1-1"
  # Midpoint overlap case.
  run_case "p3-edge-query-${fixture_name%.sam}-double-overlap" "${source_bam}" "${fixture_name}" "chr1:5-5"
  # Last reference base.
  run_case "p3-edge-query-${fixture_name%.sam}-last-base" "${source_bam}" "${fixture_name}" "chr1:32-32"
}

run_fixture_cases "p3_region_reads.sam"
run_fixture_cases "p3_region_reads_edge2.sam"

summary_json="${report_dir}/p3-indexed-edge-query-diff.json"
{
  echo "{"
  echo "  \"passed\": ${passed},"
  echo "  \"failed\": ${failed},"
...
if [[ "${failed}" -gt 0 ]]; then
  echo "P3 indexed edge query differential checks failed: ${failed}"
  exit 1
fi

echo "P3 indexed edge query differential checks passed: ${passed}, skipped: ${skipped}"
