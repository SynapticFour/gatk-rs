#!/usr/bin/env bash
# L12 production sign-off gate battery (extends L11 + PL taxonomy artifact).
#
...
#   L9_REGEN_RUST=1 L9_RUN_P12=1 ./scripts/parity/run_l12_signoff_gates.sh
...
log="${repo_root}/parity/reports/l12_signoff_gates_${ts}.log"
mkdir -p "${repo_root}/parity/reports"

{
  echo "=== L12 sign-off gates ${ts} ==="
  ...
  L9_REGEN_RUST="${L9_REGEN_RUST:-0}" L9_RUN_P12="${L9_RUN_P12:-0}" \
    ./scripts/parity/run_l11_signoff_gates.sh
  echo ""
  echo "=== L12 sign-off gates PASS ==="
  echo "log: ${log}"
