#!/usr/bin/env bash
# Phase E.7.5 gate: recoverHaplotypesFromEdgesNotCoveredInJunctionTrees vs frozen goldens.
set -euo pipefail
...
if [[ "${PARITY_SKIP_HC_FULL_E7_EDGES:-0}" == "1" ]]; then
  echo "[hc-full-parity-e7-edges] skipped (PARITY_SKIP_HC_FULL_E7_EDGES=1)"
  exit 0
fi

cases_tsv="${repo_root}/parity/fixtures/hc-full-parity/e7-edges/cases.tsv"
tmp_dir="${repo_root}/parity/reports/hc-full-parity-e7-edges-tmp"
mkdir -p "${tmp_dir}"
...
while IFS=$'\t' read -r case_id ref reads kmer minq recover_edges max_haps expected; do
  ...
  if ! cmp -s "${repo_root}/${expected}" "${actual}"; then
    echo "[hc-full-parity-e7-edges] mismatch case=${case_id}" >&2
    diff -u "${repo_root}/${expected}" "${actual}" >&2 || true
    exit 1
  fi
  echo "[hc-full-parity-e7-edges] ok ${case_id}"
done <"${cases_tsv}"

echo "[hc-full-parity-e7-edges] all cases match goldens."
