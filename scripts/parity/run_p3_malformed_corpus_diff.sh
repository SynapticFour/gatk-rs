...
{
  "label": "${label}",
  "mode": "phase3-malformed-corpus-diff",
  "equal": null,
  "skipped": true,
...
# Valid control cases.
run_case "p3-malformed-control-sam-valid" 1 \
...
run_case "p3-malformed-control-vcf-valid" 1 \
...
# Negative corpus.
run_case "p3-malformed-bam-bad-magic" 0 \
...
run_case "p3-malformed-sam-short-record" 0 \
...
run_case "p3-malformed-vcf-short-record" 0 \
...
run_case "p3-malformed-vcf-bad-pos" 0 \
...
summary_json="${report_dir}/p3-malformed-corpus-diff.json"
{
  echo "{"
  echo "  \"passed\": ${passed},"
  echo "  \"failed\": ${failed},"
...
if [[ "${failed}" -gt 0 ]]; then
  echo "P3 malformed corpus differential checks failed: ${failed}"
  exit 1
fi

echo "P3 malformed corpus differential checks passed: ${passed}, skipped: ${skipped}"
