#!/usr/bin/env bash
# Phase E.8: SeqGraph cleanup summary vs frozen goldens.
set -euo pipefail
...
if [[ "${PARITY_SKIP_HC_FULL_E8_SEQGRAPH:-0}" == "1" ]]; then
  echo "[hc-full-parity-e8-seqgraph] skipped (PARITY_SKIP_HC_FULL_E8_SEQGRAPH=1)"
  exit 0
fi

cases_tsv="${repo_root}/parity/fixtures/hc-full-parity/e8-seqgraph/cases.tsv"
tmp_dir="${repo_root}/parity/reports/hc-full-parity-e8-seqgraph-tmp"
mkdir -p "${tmp_dir}"
...
while IFS=$'\t' read -r case_id ref reads kmer minq min_prune min_dang recover_heads expected; do
  ...
  "${cargo_run[@]}" assembly-seqgraph-summary \
    "${repo_root}/${ref}" \
    "${repo_root}/${reads}" \
    "${kmer}" "${minq}" "${min_prune}" "${min_dang}" "${recover_heads}" \
    >"${actual}" 2>"${tmp_dir}/${case_id}.stderr"
  if ! cmp -s "${repo_root}/${expected}" "${actual}"; then
    echo "[hc-full-parity-e8-seqgraph] mismatch case=${case_id}" >&2
    diff -u "${repo_root}/${expected}" "${actual}" >&2 || true
    exit 1
  fi
  echo "[hc-full-parity-e8-seqgraph] ok ${case_id}"
done <"${cases_tsv}"

echo "[hc-full-parity-e8-seqgraph] all cases match goldens."
