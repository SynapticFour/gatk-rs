#!/usr/bin/env bash
# Phase B.5.1 — full read records per assembly region (GAP-B-01).
set -euo pipefail
...
if [[ "${PARITY_SKIP_HC_FULL_B5_READS:-0}" == "1" ]]; then
  echo "[hc-full-parity-b5-reads] skipped (PARITY_SKIP_HC_FULL_B5_READS=1)"
  exit 0
fi

cases_tsv="${repo_root}/parity/fixtures/hc-full-parity/b5-reads/cases.tsv"
report_dir="${repo_root}/parity/reports"
tmp_dir="${report_dir}/hc-full-parity-b5-reads-tmp"
mkdir -p "${tmp_dir}"
...
while IFS=$'\t' read -r case_id ref bam interval padding expected; do
  ...
  if [[ -z "${padding}" || "${padding}" == "-" ]]; then
    "${cargo_run[@]}" assembly-region-reads \
  ...
  if [[ "${dump_ec}" -ne 0 ]]; then
    echo "[hc-full-parity-b5-reads] dump failed (case=${case_id})" >&2
    cat "${tmp_dir}/${case_id}.stderr" >&2
    exit 1
  fi
  if ! cmp -s "${repo_root}/${expected}" "${actual}"; then
    echo "[hc-full-parity-b5-reads] mismatch case=${case_id}" >&2
    diff -u "${repo_root}/${expected}" "${actual}" >&2 || true
    exit 1
  fi
  echo "[hc-full-parity-b5-reads] ok ${case_id}"
done <"${cases_tsv}"

echo "[hc-full-parity-b5-reads] all cases match goldens."
