#!/usr/bin/env bash
# Phase C.1 gate: per-locus raw ActivityProfileState TSV vs frozen goldens.
set -euo pipefail
...
if [[ "${PARITY_SKIP_HC_FULL_C1:-0}" == "1" ]]; then
  echo "[hc-full-parity-c1] skipped (PARITY_SKIP_HC_FULL_C1=1)"
  exit 0
fi

cases_tsv="${repo_root}/parity/fixtures/hc-full-parity/c1/cases.tsv"
report_dir="${repo_root}/parity/reports"
tmp_dir="${report_dir}/hc-full-parity-c1-tmp"
mkdir -p "${tmp_dir}"
...
while IFS=$'\t' read -r case_id ref bam interval expected; do
  ...
  if [[ "${dump_ec}" -ne 0 ]]; then
    echo "[hc-full-parity-c1] dump failed (case=${case_id}, exit=${dump_ec}). stderr:" >&2
    cat "${tmp_dir}/${case_id}.stderr" >&2
    exit 1
  fi
  if ! cmp -s "${repo_root}/${expected}" "${actual}"; then
    echo "[hc-full-parity-c1] mismatch case=${case_id}" >&2
    echo "  expected: ${repo_root}/${expected}" >&2
    echo "  actual:   ${actual}" >&2
    diff -u "${repo_root}/${expected}" "${actual}" >&2 || true
    exit 1
  fi
  echo "[hc-full-parity-c1] ok ${case_id}"
done <"${cases_tsv}"

echo "[hc-full-parity-c1] all cases match goldens."
