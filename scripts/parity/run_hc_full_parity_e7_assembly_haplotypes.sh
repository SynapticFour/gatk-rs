#!/usr/bin/env bash
# Phase E.7 gate: ReadThreadingAssembler + KBest haplotype set vs frozen goldens.
set -euo pipefail
...
if [[ "${PARITY_SKIP_HC_FULL_E7:-0}" == "1" ]]; then
  echo "[hc-full-parity-e7] skipped (PARITY_SKIP_HC_FULL_E7=1)"
  exit 0
fi

cases_tsv="${repo_root}/parity/fixtures/hc-full-parity/e7/cases.tsv"
report_dir="${repo_root}/parity/reports"
tmp_dir="${report_dir}/hc-full-parity-e7-tmp"
mkdir -p "${tmp_dir}"
...
while IFS=$'\t' read -r case_id ref reads kmer minq min_prune min_dang recover_heads expected; do
  ...
  "${cargo_run[@]}" assembly-haplotypes \
    ...
    "${min_prune}" \
    "${min_dang}" \
    "${recover_heads}" >"${actual}" 2>"${tmp_dir}/${case_id}.stderr"
  dump_ec=$?
  set -e
  if [[ "${dump_ec}" -ne 0 ]]; then
    echo "[hc-full-parity-e7] dump failed (case=${case_id}, exit=${dump_ec}). stderr:" >&2
    cat "${tmp_dir}/${case_id}.stderr" >&2
    exit 1
  fi
  if ! cmp -s "${repo_root}/${expected}" "${actual}"; then
    echo "[hc-full-parity-e7] mismatch case=${case_id}" >&2
    echo "  expected: ${repo_root}/${expected}" >&2
    echo "  actual:   ${actual}" >&2
    diff -u "${repo_root}/${expected}" "${actual}" >&2 || true
    exit 1
  fi
  echo "[hc-full-parity-e7] ok ${case_id}"
done <"${cases_tsv}"

echo "[hc-full-parity-e7] all cases match goldens."
