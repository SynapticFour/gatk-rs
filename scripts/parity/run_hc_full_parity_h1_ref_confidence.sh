#!/usr/bin/env bash
# Phase H.1.1 — ReferenceConfidenceModel per-locus dump vs frozen goldens.
set -euo pipefail
...
cases_tsv="${repo_root}/parity/fixtures/hc-full-parity/h1/cases.tsv"
tmp_dir="${repo_root}/parity/reports/hc-full-parity-h1-tmp"
mkdir -p "${tmp_dir}"
...
while IFS=$'\t' read -r case_id ref bam interval padding expected; do
  ...
  "${cargo_run[@]}" reference-confidence-locus \
    "${repo_root}/${ref}" "${repo_root}/${bam}" "${interval}" "${padding}" >"${actual}"
  cmp -s "${repo_root}/${expected}" "${actual}" || {
    echo "[hc-full-parity-h1] mismatch ${case_id}" >&2
    diff -u "${repo_root}/${expected}" "${actual}" >&2 || true
    exit 1
  }
  echo "[hc-full-parity-h1] ok ${case_id}"
done <"${cases_tsv}"

echo "[hc-full-parity-h1] all cases match goldens."
