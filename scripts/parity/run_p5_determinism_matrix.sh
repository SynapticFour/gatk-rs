...
for t in "${threads[@]}"; do
  for r in "${repeats[@]}"; do
    ...
    set +e
    out=$(cd "${repo_root}" && RAYON_NUM_THREADS="${t}" cargo test -p gatk-haplotypecaller --test p5_assembly_regression_test --locked outputs_are_stable_across_repeated_runs_and_input_order 2>&1)
    code=$?
    set -e
    ok_int=0
    if [[ "${code}" -eq 0 ]]; then
