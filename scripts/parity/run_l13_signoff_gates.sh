#!/usr/bin/env bash
# L13 production sign-off gate battery (extends L12 + InformativeAd taxonomy).
#
...
#   L9_REGEN_RUST=1 L9_RUN_P12=1 ./scripts/parity/run_l13_signoff_gates.sh
...
log="${repo_root}/parity/reports/l13_signoff_gates_${ts}.log"
mkdir -p "${repo_root}/parity/reports"

{
  echo "=== L13 sign-off gates ${ts} ==="
  echo "CARGO_TARGET_DIR=${CARGO_TARGET_DIR}"
  python3 "${repo_root}/scripts/parity/excellence_gates_audit.py"
  # Informational soft-PL taxonomy (does not fail the battery; do not clobber L13_PL_TAXONOMY.md)
  python3 "${repo_root}/scripts/parity/l12_pl_taxonomy.py" || true
  L9_REGEN_RUST="${L9_REGEN_RUST:-0}" L9_RUN_P12="${L9_RUN_P12:-0}" \
    ./scripts/parity/run_l12_signoff_gates.sh
  echo ""
  echo "=== L13 sign-off gates PASS ==="
  echo "log: ${log}"
