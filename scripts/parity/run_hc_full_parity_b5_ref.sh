#!/usr/bin/env bash
# Phase B.5.3 — ReferenceContext padded-span bases per assembly region.
set -euo pipefail
...
if [[ "${PARITY_SKIP_HC_FULL_B5_REF:-0}" == "1" ]]; then
  echo "[hc-full-parity-b5-ref] skipped (PARITY_SKIP_HC_FULL_B5_REF=1)"
  exit 0
fi

cases_tsv="${repo_root}/parity/fixtures/hc-full-parity/b5-ref/cases.tsv"
report_dir="${repo_root}/parity/reports"
tmp_dir="${report_dir}/hc-full-parity-b5-ref-tmp"
mkdir -p "${tmp_dir}"
...
while IFS=$'\t' read -r case_id ref bam interval padding expected; do
  ...
  if [[ -z "${padding}" || "${padding}" == "-" ]]; then
    "${cargo_run[@]}" assembly-region-reference \
  ...
  if [[ "${dump_ec}" -ne 0 ]]; then
    echo "[hc-full-parity-b5-ref] dump failed (case=${case_id})" >&2
    cat "${tmp_dir}/${case_id}.stderr" >&2
    exit 1
  fi
  if ! cmp -s "${repo_root}/${expected}" "${actual}"; then
    echo "[hc-full-parity-b5-ref] mismatch case=${case_id}" >&2
    diff -u "${repo_root}/${expected}" "${actual}" >&2 || true
    exit 1
  fi
  echo "[hc-full-parity-b5-ref] ok ${case_id}"
done <"${cases_tsv}"

echo "[hc-full-parity-b5-ref] all cases match goldens."
