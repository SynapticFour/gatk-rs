//! 6R.177 live: InbreedingCoeff at `2:92305634 G/T` (6R.178 omits when n<10).
//! Skipped unless `HOLDOUT_6R177=1`.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R177\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r177_inbreeding() {
    if std::env::var("HOLDOUT_6R177").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R177=1");
        return;
    }
    let root = repo_root();
    ...
    kv(
        "first_arrow",
        "Java InbreedingCoeff emptyMap when n<MIN_SAMPLES=10; 6R.178 matches that omit",
    );
