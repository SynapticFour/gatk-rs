//! 6R.171 live: ReadPosRankSum still inserted as 0; FORMAT/FS/SOR/MQ unchanged.
//! Skipped unless `HOLDOUT_6R171=1`. Production change: NONE.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R171\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r171_read_pos_rank_sum_empty() {
    if std::env::var("HOLDOUT_6R171").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R171=1");
        return;
    }
    let root = repo_root();
    ...
    kv(
        "java_empty",
        "REF=0 ALT=3 → MannWhitneyU NaN → emptyMap → no ReadPosRankSum",
    );
