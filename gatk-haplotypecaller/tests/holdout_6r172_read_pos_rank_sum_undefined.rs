//! 6R.172 live: FORMAT/FS/SOR/MQ unchanged; ReadPosRankSum emit predicate is Option.
//! Skipped unless `HOLDOUT_6R172=1`. Evidence source remains 6R.170 pileup.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R172\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r172_read_pos_rank_sum_undefined() {
    if std::env::var("HOLDOUT_6R172").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R172=1");
        return;
    }
    let root = repo_root();
    ...
    kv(
        "production_change",
        "ReadPosRankSum Option<f64>; Some(0.0) emits; None omits",
    );
    ...
    assert!(
        ...
        "FS=0 must remain; skip-zero would regress"
    );
    assert!(fs < 0.02);
