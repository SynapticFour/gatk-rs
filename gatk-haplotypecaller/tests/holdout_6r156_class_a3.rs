//! 6R.156: Class-A3 semantic contract on the frozen 20:29456196 A/T site.
//! Diagnostic-only `unbounded_diagnostic`. Skipped unless `HOLDOUT_6R156=1`.
//! Does not set `GATK_RS_DIAGNOSTIC_SKIP_SITE_RESHAPE` (production result unchanged).
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R156\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r156_class_a3_semantic_contract() {
    if std::env::var("HOLDOUT_6R156").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R156=1");
        return;
    }
    assert_ne!(
        std::env::var("GATK_RS_DIAGNOSTIC_SKIP_SITE_RESHAPE")
            ...
            .as_deref(),
        Some("1"),
        "6R.156 must not alter the default production result"
    );
    ...
    if let Some(c) = prod {
        assert_ne!(
            c.genotype.format.pl_as_i32(),
            vec![81, 0, 36],
            "6R.158: A3 must not sparse-replace"
        );
