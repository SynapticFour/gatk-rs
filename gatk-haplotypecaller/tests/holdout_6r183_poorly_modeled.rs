//! 6R.183 live: stored hap likelihoods at `2:92307333 T/G` are n=2 after 6R.185.
//! after a Java-equivalent poorly-modeled pass of n=2 (proof-only).
//! Skipped unless `HOLDOUT_6R183=1`. Production change: NONE.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R183\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r183_poorly_modeled() {
    if std::env::var("HOLDOUT_6R183").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R183=1");
        return;
    }
    let root = repo_root();
    ...
    assert_eq!(extra_n, 0, "extras KEEP is not causal at this site");
    ...
    assert_eq!(stored.len(), 2, "6R.185 stored hap matrix is Java n=2");
