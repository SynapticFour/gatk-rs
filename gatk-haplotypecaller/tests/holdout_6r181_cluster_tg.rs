//! 6R.181 live: INFO DP/MQ/SOR at `2:92307333 T/G` still region-wide (proof-only).
//! Skipped unless `HOLDOUT_6R181=1`. Production change: NONE.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R181\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r181_cluster_tg() {
    if std::env::var("HOLDOUT_6R181").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R181=1");
        return;
    }
    let root = repo_root();
