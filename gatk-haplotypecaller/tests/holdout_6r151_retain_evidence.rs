//! 6R.151: Java-equivalent `retainEvidence` without QNAME collapse after the
//! 6R.150 201→41 overlap object. Diagnostic-only `unbounded_diagnostic`.
//! Skipped unless `HOLDOUT_6R151=1`.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R151\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r151_retain_evidence_mate_independence() {
    if std::env::var("HOLDOUT_6R151").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R151=1");
        return;
    }
    ...
    let (classification, first_divergence) = if !predicate_equal {
        ...
        (
            "QNAME_DEDUPE_JAVA_INCOMPATIBLE",
            format!(
                "production retainEvidence still diverges JAVA_ONLY={} RUST_ONLY={}",
                prod_java_only.len(),
                prod_rust_only.len()
            ),
        )
    } else if helper_java_only.len() != 1 || helper_collapsed.len() != 40 {
        (
            "QNAME_DEDUPE_JAVA_INCOMPATIBLE",
            format!(
                "helper collapse did not reproduce the 6R.150 41→40 JAVA_ONLY={}",
                helper_java_only.len()
            ),
        )
        ...
        (
            ...
            format!(
                "41-read likelihood max_abs_delta={max_abs:.6e} gl_delta={gl_retain_delta:.6e}"
            ),
        )
    } else {
        (
            "RETAIN_EVIDENCE_FIX_PROVEN",
            format!(
                "NONE (helper would drop flags=99 mate JAVA_ONLY={} collapse_changes_gl_winner={collapse_changes_winner} gl_margin={collapse_margin:.6e}; production keeps 41)",
                helper_java_only.len()
            ),
        )
    };
    ...
    kv(
        "production_change",
        "bypass dedupe_likelihood_subset_by_qname on Java-strict retainEvidence path; keep helper for sparse/P12 augment and empty-subset rescue",
    );
    kv(
        "next_arrow",
        "genotype-prior construction (6R.150); not AD/QUAL/L9/VCF",
    );
    ...
    assert_eq!(helper_collapsed.len(), 40);
    assert!(
        predicate_equal && production_equal,
        "Java/Rust retainEvidence membership must match without QNAME collapse"
    );
    assert!(
        java_retained.contains(&mate_first),
        "Java keeps flags=99 mate"
    );
    assert!(
        rust_prod_retained.contains(&mate_first),
        "production must keep the previously dropped flags=99 mate"
    );
    assert!(
        !helper_collapsed.contains(&mate_first),
        "helper still drops the flags=99 mate"
    );
    assert_eq!(mate_keys.len(), 2, "Java evidence includes both mates");
    assert!(
        max_abs < 1e-4,
        "retained allele-likelihood residual must stay in the Java-float / Rust-f64 band"
    );
    assert_eq!(classification, "RETAIN_EVIDENCE_FIX_PROVEN");
}
