//! 6R.179 live: INFO DP/MQ/SOR at `2:92307324 TTC/T` (proof-only).
//! Skipped unless `HOLDOUT_6R179=1`. Production change: NONE.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R179\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r179_indel_info() {
    if std::env::var("HOLDOUT_6R179").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R179=1");
        return;
    }
    let root = repo_root();
    ...
    kv("java_info", "DP=1 MQ=44.00 SOR=1.609");
    kv("rust_info", format!("DP=1 MQ={mq} SOR={sor}"));
    kv(
        "first_arrow",
        "6R.180 closed region-wide vs per-variant annotation AlleleLikelihoods",
    );
}
