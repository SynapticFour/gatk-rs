//! 6R.175 live: after 6R.176, SOR at `2:92305634 G/T` is Java 0.693.
//! Skipped unless `HOLDOUT_6R175=1`.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R175\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r175_sor() {
    if std::env::var("HOLDOUT_6R175").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R175=1");
        return;
    }
    let root = repo_root();
    ...
    kv(
        "production_change",
        "NONE in 6R.175; 6R.176 closed SOR membership",
    );
    kv("java_sor", "0.693");
    kv("java_table", "[0,0;1,1]");
    kv("rust_table", "[0,0;1,1]");
