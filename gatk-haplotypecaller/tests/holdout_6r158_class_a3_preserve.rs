//! 6R.158: Class-A3 preserves the assigned calculator genotype on the frozen
//! 20:29456196 A/T site. Diagnostic-only `unbounded_diagnostic`.
//! Skipped unless `HOLDOUT_6R158=1`.
//!
//! ```text
...
fn kv(key: &str, value: impl AsRef<str>) {
    println!("6R158\t{key}\t{}", value.as_ref());
}
...
#[test]
fn holdout_6r158_class_a3_preserves_calculator() {
    if std::env::var("HOLDOUT_6R158").ok().as_deref() != Some("1") {
        eprintln!("skip: set HOLDOUT_6R158=1");
        return;
    }
    assert_ne!(
        std::env::var("GATK_RS_DIAGNOSTIC_SKIP_SITE_RESHAPE")
            ...
            .as_deref(),
        Some("1"),
        "6R.158 must not alter the default production result via the diagnostic skip"
    );
    ...
    kv(
        "gq_dependency",
        "calculator GQ from PL gap (5); A3 no longer rewrites GQ from SparsePlShape",
    );
    kv(
        "dp_dependency",
        "calculator DP = informative AD sum 41; A3 no longer copies pileup DP=46",
    );
    kv("classification", "A3_CALCULATOR_GENOTYPE_PRESERVATION");
    kv("production_change", "YES");
    ...
    assert_eq!(cf_a.pl_as_i32(), vec![0, 5, 1174]);
    assert_eq!(cf_a.ad_as_i32(), vec![22, 24]);
    assert_eq!(cf_a.gq.as_i32(), 5);
    assert_eq!(cf_b.pl_as_i32(), vec![81, 0, 36]);
    assert_eq!(cf_b.ad_as_i32(), vec![37, 4]);
    assert_eq!(cf_c.pl_as_i32(), vec![0, 5, 1174]);
    assert_eq!(cf_c.ad_as_i32(), vec![37, 4]);
    assert_eq!(cf_d.pl_as_i32(), vec![0, 5, 1174]);
    match prod {
        Some(c) => {
            ...
            assert_eq!(c.genotype.format.gq.as_i32(), 5);
            assert_eq!(c.genotype.format.dp.as_i32(), 41);
        ...
        None => {
            kv(
                "next_divergence",
                "genotyped_calls absent after A3 skip (hom-ref emit gate); Java emits 0/0 0,5,1174 37,4",
            );
        }
    }
}
